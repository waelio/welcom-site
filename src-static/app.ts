...
const AUTHENTICATE_JASON_PORTAL_PAYLOAD =
    "welcomtalk://portal-start?n=Jordan%20Smith&t=Repair%20dispute&s=Scan%20with%20my%20iPhone%20to%20start%20the%20session.";
...
const setupRequestBuilder = (): void => {
    ...
    authenticateJasonButton.addEventListener("click", () => {
        ...
        writeRecordPreview(
            record,
            "Jason is ready. Scan the barcode in WelcomTalk on your iPhone to start the session.",
        );
        showBarcodeCard();

        const clipboardWrite = navigator.clipboard?.writeText(
            AUTHENTICATE_JASON_PORTAL_PAYLOAD,
        );
